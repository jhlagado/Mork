; *************************************************************************
;
;       Mork Minimal Interpreter for the Z80 
;
...
;
; *****************************************************************************
    TRUE        EQU 1		; not FF, for Mork
    FALSE       EQU 0
    EMPTY       EQU 0		; for an empty macro, ctrl-<something>=macro, ie ctrl-h = backspace macros (in Mork)
...
; **************************************************************************		
	...
	.ORG ROMSTART + $180		; 0+180 put Mork code from here	

; **************************************************************************
; Macros must be written in Mork and end with ; 
; this code must not span pages
; **************************************************************************
macros:

backsp_:
    DB "\\t@0=0=(\\t@1-\\t!`\b \b`);"	;ctr-h  , \ needed inside control code, escape it with anothe \
...
iOpcodes:
    ...
    DB     lsb(aNop_)       ;dd     \]
    DB     lsb(go_)         ;de     \^  ( -- ? ) execute Mork definition a is address of Mork code
    ...
    LITDAT 5
    DB    lsb(rpop_)        ;       { ( -- n ) pop from Mork return stack 
    DB    lsb(aNop_)        ;                  
...
macro:                          ;=25
    ...
    LD D,msb(macros)
    PUSH DE
    call ENTER		;Mork go operation and jump to it
    .cstr "\\^"
macro1:
    LD BC,(vTIBPtr)
    JR interpret2
...
utilTable:
    ...
    DB lsb(prompt_)     ;\#5    ( -- )        print Mork prompt 
